...
class Categorie(db.Model):
    """Représente les genres musicaux (Jazz, Rock, Electro)"""
...
class Concert(db.Model):
    """Gère les événements, les réservations et les archives passées"""
    id = db.Column(db.Integer, primary_key=True)
    ...
    # Nouveaux champs pour les concerts passés
...
class Utilisateur(db.Model, UserMixin):
    """Gère les administrateurs et les visiteurs connectés"""
    id = db.Column(db.Integer, primary_key=True)
...
class Commentaire(db.Model):
    """Stocke les commentaires des utilisateurs sur les concerts passés"""
    id = db.Column(db.Integer, primary_key=True)
    contenu = db.Column(db.Text, nullable=False)
